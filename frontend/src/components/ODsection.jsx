...
const ODSection = () => {
  ...
  // Updated getStatusIcon function to show individual approval statuses
  const getStatusIcon = (approvalStatus) => {
    // If explicitly approved
    if (approvalStatus === true) {
      return <FaCheckCircle className="text-green-500" title="Approved" />;
    }
    // If explicitly rejected (we need to check if this specific role rejected it)
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-3 md:p-6 rounded-lg shadow-lg border-2 border-gray-900 dark:border-gray-600">
      <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-4">Pending OD Requests</h2>
